...
# produce a chapter_title map via 114 get requests
...
def translation_instantiator(author_name, translation_language, translation_filepath, progress_counter, progress_denominator)
  ...
  File.open( translation_filepath ).each do |line|
    ...
    if current_chapter
      if current_chapter.number != chapter_number.to_i
        current_chapter = Chapter.create(translation_id: current_translation.id, number: chapter_number, title: chapter_title)
      end
    else
      current_chapter = Chapter.create(translation_id: current_translation.id, number: chapter_number, title: chapter_title)
    end
...
# NoMethodError: undefined method `split' for nil:NilClass
  # try to return out of the parser once a line is a blank line
